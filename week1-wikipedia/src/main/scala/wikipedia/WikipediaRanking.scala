...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.filter(article => isLangMentionedInArticle(lang, article))
      .count()
      .toInt
  }

  private def isLangMentionedInArticle(lang: String, article: WikipediaArticle): Boolean = {
    article.text.split("\\s+").contains(lang)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(article => langs.collect { case lang if isLangMentionedInArticle(lang, article) => (lang, article) })
      .groupByKey()
  }
